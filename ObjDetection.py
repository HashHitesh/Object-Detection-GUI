...
# Main Window
root = tk.Tk()
root.title(" Tensorflow Object Detection GUI")
...
def noPhotoSelected():
   tk.messagebox.showinfo( "Warning! ", "Please upload a photo via ** Upload Photo ** button ! ")
...
def showTheImages():
    resized = PIL.Image.open(str(filepath[thePhotoWeWorkingOn])) 
    maxsize = (photoPanel.winfo_width() - 50,photoPanel.winfo_height() - 50 )
    resized.thumbnail(maxsize, PIL.Image.ANTIALIAS)
...
def open_img():
    if photo_selected == 1:
        ...
        resized = PIL.Image.open('D:\PL\AI Model GUI\BasicGuiDesign\imgWeWorkingOn/temp.jpg') 
        maxsize = (photoPanel.winfo_width() - 50,photoPanel.winfo_height() - 50 )
        resized.thumbnail(maxsize, PIL.Image.ANTIALIAS)
        img = PIL.ImageTk.PhotoImage(resized)
        photoPanel.config(image = img)
